...
struct WeeklyRuleEditView: View {
    ...
    init(mode: WeeklyRuleEditMode, overrideStore: OverrideStore, onSave: @escaping (WeeklyRule) -> Void, onDelete: (() -> Void)? = nil) {
        self.mode = mode
        self.onSave = onSave
...
#Preview {
    WeeklyRuleEditView(mode: .add, overrideStore: OverrideStore(), onSave: { _ in })
